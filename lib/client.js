...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      // ---- mobile responsive layout (phone portrait) ----
      '@media (max-width: 768px) {',
      '  .dsh-bal-hdr { gap: 4px; max-width: 100%; min-width: 0; }',
      '  .dsh-bal-badges { display: none; }',
      '  .dsh-bal-chip { height: 28px; padding: 0 7px; font-size: 11px; max-width: 130px; overflow: hidden; text-overflow: ellipsis; }',
      '  .dsh-bal-overlay { top: calc(env(safe-area-inset-top, 0px) + 52px); right: 8px; width: calc(100vw - 16px); max-width: none; }',
      '}',
      '@media (max-width: 480px) {',
      '  .dsh-bal-clabel { display: none; }',
      '  .dsh-bal-chip-refresh { display: none; }',
      '  .dsh-download-label { display: none; }',
      '}',
    ].join('\n');
    ...
    function apply(ctx) {
      ...
      function useBalance() {
        ...
        var v = state[0];
        var setV = state[1];
        // store 是模块级单例，setStore 原地修改后若直接传同一引用，
        // React 的 Object.is 比较会跳过重渲染（bail out）——必须传浅拷贝。
        React.useEffect(function () { return subscribe(function () { setV(Object.assign({}, store)); }); }, []);
        return v;
      }
      ...
      function DownloadCapsule(props) {
        ...
        return React.createElement('button', {
          ...
          React.createElement('span', { className: 'dsh-download-label' }, '下载'),
          React.createElement('span', { className: 'dsh-download-icon', 'aria-hidden': true }, '⬇')
        );
      }
      ...
      slots.inject('conversation.session.header.utilities', function () {
        return slots.register(
          { name: 'conversation.session.header.utilities', id: 'session-log-download', priority: -1, order: 0 },
